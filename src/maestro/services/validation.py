...
class ReleaseValidationService:
    ...
    async def validate(self, config: ReleaseConfigSchema, session=None) -> None:
        ...
        db_session = session if session is not None else AsyncSessionLocal()
        registry_repo = JobPathRegistryRepository(db=db_session)

        try:
            await self._init_integrations(db_session)
            errors: List[str] = []
            ...
            for stage in config.spec.stages:
                for step in stage.steps:
                    ...
                    if job_type == "jenkins":
                        job_path = await resolve_job_path_async(step, config.spec, registry_repo)
