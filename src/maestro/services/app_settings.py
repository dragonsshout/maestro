...
async def get_integration_settings(session=None) -> dict[str, str | None]:
    """
    ...
    :returns: Dicionário com as configurações.
    """
    ...
    try:
        ...
        def _get(key: str, env_fallback: str | None) -> str | None:
            raw = saved.get(key)
            if raw:
